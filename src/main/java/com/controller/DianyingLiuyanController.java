...
//            dianyingLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<DianyingLiuyanEntity> queryWrapper = new EntityWrapper<DianyingLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        DianyingLiuyanEntity dianyingLiuyanEntity = dianyingLiuyanService.selectOne(queryWrapper);
        dianyingLiuyan.setUpdateTime(new Date());
        if(dianyingLiuyanEntity==null){
...
//                            dianyingLiuyanEntity.setDianyingId(Integer.valueOf(data.get(0)));   //电影 要改的
//                            dianyingLiuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            dianyingLiuyanEntity.setDianyingLiuyanText(data.get(0));                    //影评内容 要改的
//                            dianyingLiuyanEntity.setReplyText(data.get(0));                    //回复内容 要改的
//                            dianyingLiuyanEntity.setInsertTime(date);//时间
//                            dianyingLiuyanEntity.setUpdateTime(new Date(data.get(0)));          //回复时间 要改的
